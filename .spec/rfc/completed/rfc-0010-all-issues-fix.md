...
**状态**: 历史完成 (Historical Completed)  
**作者**: 开发团队  
**创建日期**: 2025-01-16  
**最后更新**: 2026-04-25 (归档：后续 RFC 和全量构建已收敛)

> **当前使用方式**：本文保留为早期“全面修复编译问题”的历史记录，不再作为当前待办。仍有价值的是错误分类和修复脉络；当前可执行工作以 [TASK_TRACKING](../../../TASK_TRACKING.md) 为准，路径/输出契约见 [RFC-0011](./rfc-0011-windows-repository-layout.md)，第三方库和构建门闩见 [RFC-0012](./rfc-0012-thirdparty-library-upgrades.md)。
...
- [x] 修复 `TCHAR`/`wchar_t` 混用（由后续编译修复和全量构建验证收敛）
- [x] 统一使用宽字符 API（由后续编译修复和全量构建验证收敛）
...
- [x] 修复 `av_h264_decode_frame` 未解析（由后续编译修复和全量构建验证收敛）
- [x] 修复 `wcscpy` 等函数的安全警告（由后续编译修复和全量构建验证收敛）
- [x] 修复 `std::basic_ostream` 未定义（由后续编译修复和全量构建验证收敛）
- [x] 修复 `boost/filesystem.hpp` 找不到（由后续编译修复和全量构建验证收敛）
- [x] 修复 `pcid/pcid/PCIDCalculator.h` 路径错误（由后续编译修复和全量构建验证收敛）
- [x] 修复大量 TCHAR/wchar_t 混用错误（由后续编译修复和全量构建验证收敛）
