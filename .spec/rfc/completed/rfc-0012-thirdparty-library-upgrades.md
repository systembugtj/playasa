...
| 字段 | 内容 |
|------|------|
| **状态** | 已完成 (Completed)：P1-P5 均已落地，后续 curl 下载恢复拆分到 RFC-0015 |
| **适用范围** | 本仓库 `playasa`（MSBuild / Win32 为主） |
| **相关 RFC** | [RFC-0011](./rfc-0011-windows-repository-layout.md)（路径与 `out\` 契约）、[RFC-0002](./rfc-0002-build-environment-analysis.md)（历史环境分析）、[RFC-0015](../rfc-0015-curl-schannel-updater-download.md)（curl + Schannel 下载恢复） |
...
| `src/Thirdparty/openssl-0.9.8x` | OpenSSL | **已移除** | 活跃 VS 工程禁止 `libeay32.lib` / `ssleay32.lib`；Updater Debug 删除缺失的旧 `curllibd.lib` 链接并保留 Schannel 所需系统库 |
...
| `src/Thirdparty/librhash` | RHash / librhash | **已对齐上游 v1.4.6**（`RHASH_HASH_COUNT = 32`）；本仓 `rhash_ex.cpp` 封装 ED2K/磁力 | `plug_openssl.c` 保留上游可选代码，但工程未定义 `USE_OPENSSL` / `OPENSSL_RUNTIME`，不链接 OpenSSL |
...
| 2026-04-25 | **P5 执行**：删除 OpenSSL 0.9.8x 树，清理旧 curl/OpenSSL 链接 | `mplayerc` Debug Unicode 移除 `libeay32.lib`/`ssleay32.lib`；`Updater` Debug 移除 OpenSSL 与缺失的 `curllibd.lib`，保留 `Crypt32.lib`、`Secur32.lib`、`Wldap32.lib`、`Normaliz.lib`、`Ws2_32.lib`；`openssl-rfc0012-expected.txt` 与 `verify-rfc0012-p5-openssl-audit.ps1` 钉住“OpenSSL 已移除” |
...
**当前执行结论**：`src/Thirdparty/openssl-rfc0012-expected.txt` = `openssl-0.9.8x-dropped`。`src/Thirdparty/openssl-0.9.8x` 已删除；活跃 `.vcxproj` 中的 `mplayerc`（`Release Unicode|Win32`、`Release|Win32`、`Debug Unicode|Win32`）和 `Updater`（`Release|Win32`、`Debug Unicode|Win32`、`Debug|Win32`）均不得链接 `libeay32.lib` / `ssleay32.lib`。`Updater` `Debug|Win32` 原本还链接缺失的 `curllibd.lib`，但当前源码没有 `curl_easy_*` / `CURLOPT_*` 调用，保留该链接只会造成假依赖和构建失败；因此移除旧 curl 库链接，只保留 Schannel 方向的系统库依赖（`Crypt32.lib`、`Secur32.lib`、`Wldap32.lib`、`Normaliz.lib`、`Ws2_32.lib`）。若后续恢复 libcurl 下载实现，必须引入 Schannel 后端的 libcurl 包并由本门闩扩展校验。
...
| P5 OpenSSL 移除 | 防止 OpenSSL 0.9.8x 树、活跃 OpenSSL 链接或缺失旧 curl 库静默回归 | `verify-rfc0012-p5-openssl-audit.ps1` + `Thirdparty/openssl-rfc0012-expected.txt` |
