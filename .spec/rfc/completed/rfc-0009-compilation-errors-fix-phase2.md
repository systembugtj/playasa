...
> **当前使用方式**：本文保留为第二阶段编译错误修复记录，不再作为当前待办。阶段 3/4/6 中遗留的宽字符、ATL、链接问题已被 RFC-0010 后续修复、RFC-0011 路径契约、RFC-0012 第三方升级以及最近全量 `splayer.sln` 构建验证覆盖；当前状态以 [TASK_TRACKING](../../../TASK_TRACKING.md) 为准。
...
- [x] 修复 `TCHAR`/`wchar_t` 混用（由后续编译修复收敛）
- [x] 统一使用宽字符 API（由后续编译修复收敛）

### 阶段 4: ATL 相关问题（历史关闭）

- [x] 修复 `CImage::Load` 参数类型（由后续编译修复收敛）
- [x] 修复 `CStringT` 格式化函数（由后续编译修复收敛）
...
- [x] 检查 `CSVPEqualizer` 链接（由 RFC-0010 后续修复收敛）
- [x] 检查 `Utility` 类链接（由 RFC-0010 后续修复收敛）
- [x] 检查 DirectShow 工具函数链接（由 RFC-0010 后续修复收敛）
