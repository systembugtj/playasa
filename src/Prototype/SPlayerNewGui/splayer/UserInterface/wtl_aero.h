...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
//  This is a modified version of WTL's Aero.h for to simplify usage of      //
//  composite desktop feature on Windows Vista and 7 to build up frame       //
//  window for generic usage.                                                //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
// extracted WTL's Aero.h modified to remove BufferedPaint
// routines as we don't need them in this implementation 
template <class T>
class CAeroExImpl :
  public CThemeImpl<T>
{
public:
  CAeroExImpl(LPCWSTR lpstrThemeClassList = L"globals") 
  {
    SetThemeClassList(lpstrThemeClassList);
    MARGINS m = {0};
    m_Margins = m;
...
//////////////////////////////////////////////////////////////////////////
// extracted WTL's Aero.h modified to remove BufferedPaint
// routines, toolbar and statusbar handling as we don't need 
// them in this implementation 
template <class T, class TBase = ATL::CWindow, class TWinTraits = ATL::CFrameWinTraits>
class ATL_NO_VTABLE CAeroExFrameImpl : 
  public CFrameWindowImpl<T, TBase, TWinTraits>,
  public CAeroExImpl<T>
{
  typedef CFrameWindowImpl<T, TBase, TWinTraits> _baseClass;
public:
  CAeroExFrameImpl(LPCWSTR lpstrThemeClassList = L"window") : CAeroExImpl(lpstrThemeClassList)
  {}
  ...
  BEGIN_MSG_MAP(CAeroExFrameImpl)
    CHAIN_MSG_MAP(CAeroExImpl<T>)
    CHAIN_MSG_MAP(_baseClass)
  END_MSG_MAP()
};
...
///////////////////////////////////////////////////////////////////////////////
//
//  ScopedRedrawLock class is designed specifically to deal with a Windows
//  flaw during non-client painting.
//
//  When APIs such as SetWindowText is called, Windows do not call
//  RedrawWindow with RDW_FRAME flag, and do not use SetWindowPos with 
//  SWP_FRAMECHANGED flags. Instead, it obtains the device-context of the
//  non-client area and does things directly. This causes our frame painting
//  routines in WM_NCPAINT to be skipped.
//
//  To deal with this problem, we impose a "lock" that temporarily removes
//  WS_VISIBLE window style during entrance, and put it back on when leaves.
//  This apparently make SetWindowText believes no painting should be done,
//  and happens so fast that no flicker is seen.
//
class ScopedRedrawLock {
public:
  explicit ScopedRedrawLock(HWND hWnd) : 
    m_hWnd(hWnd), m_bUpdateLocked(false), m_SavedWindowStyle(0)
  {
    LockUpdates();
  }
...
private:
  void LockUpdates() {
    ...
    m_SavedWindowStyle = GetWindowLongPtr(m_hWnd, GWL_STYLE);
    SetWindowLongPtr(m_hWnd, GWL_STYLE, m_SavedWindowStyle & ~WS_VISIBLE);
  }

  void UnlockUpdates() {
    if (m_bUpdateLocked)
    {
      SetWindowLongPtr(m_hWnd, GWL_STYLE, m_SavedWindowStyle);
      m_bUpdateLocked = false;
    }
  }

  bool m_bUpdateLocked;
  LONG_PTR m_SavedWindowStyle;
