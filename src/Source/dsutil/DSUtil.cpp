...
cdrom_t GetCDROMType(TCHAR drive, CAtlList<CString>& files, BOOL quick)
{
  files.RemoveAll();
...
{
  //TODO:  解決 [ＮＨＫ総合・東京] 第60回NHK紅白歌合戦_水樹奈々 (1440x1080i) 不能識別的問題
...
HRESULT LoadExternalObject(LPCTSTR path, REFCLSID clsid, REFIID iid, void** ppv, IUnknown *pUnkOuter)
{
  ...
  while(pos)
  {
    ...
    if(!eo.path.CompareNoCase(fullpath))// && eo.clsid == clsid
    {
      hInst = eo.hInst;
      fFound = true;
      break;
  ...
  if(hInst || (hInst = CoLoadLibrary(CComBSTR(fullpath), FALSE)))
  {
    ...
    {
      SVP_LogMsg5(L"LoadExternalObject Shouldn't be so easy %x %x %x %s %s",
        hr, p , ppv, CStringFromGUID(iid), CStringFromGUID(clsid));
      CoFreeLibrary(hInst);
    }
    else
    {
      ...
      if(SUCCEEDED(hr = p(clsid, __uuidof(IClassFactory), (void**)&pCF)))
        hr = pCF->CreateInstance(pUnkOuter, iid, ppv);
      Logging(L"DllGetClassObject success %x %x", pUnkOuter, hr);
    }
  }
  ...
  //Logging(L"CoCreate End %x %s %s %s", hr, fullpath, CStringFromGUID(iid), CStringFromGUID(clsid));
...
void UnloadExternalObjects()
{
  ...
  {
    ExternalObject& eo = s_extobjs.GetNext(pos);
    Logging(L"UnloadExternalObjects %s %x", eo.path, eo.hInst, eo.clsid);
    CoFreeLibrary(eo.hInst);
  }
  s_extobjs.RemoveAll();
  Logging(L"UnloadExternalObject end");
}
...
static struct {LPCSTR name, iso6392, iso6391; LCID lcid;} s_isolangs[] =	// TODO : fill LCID !!!
{
  ...
  {"Bini", "bin", ""},
  {"Bislama", "bis", "bi"},
  {"Bokm錶, Norwegian; Norwegian Bokm錶", "nob", "nb"},
  {"Bosnian", "bos", "bs"},
  {"Braj", "bra", ""},
  ...
  {"Norwegian Bokm錶; Bokm錶, Norwegian", "nob", "nb",	MAKELCID( MAKELANGID(LANG_NORWEGIAN, SUBLANG_DEFAULT), SORT_DEFAULT)},
  ...
  {"Nyoro", "nyo", ""},
  {"Nzima", "nzi", ""},
  {"Occitan (post 1500},; Proven鏰l", "oci", "oc",		MAKELCID( MAKELANGID(LANG_OCCITAN, SUBLANG_DEFAULT), SORT_DEFAULT)},
  {"Ojibwa", "oji", "oj"},
  ...
  {"Proven鏰l; Occitan (post 1500)", "oci", "oc"},
  {"Proven鏰l, Old (to 1500)", "pro", ""},
  {"Pushto", "pus", "ps"},
