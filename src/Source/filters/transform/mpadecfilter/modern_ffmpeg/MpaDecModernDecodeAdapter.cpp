#include "../stdafx.h"
#include "MpaDecModernDecodeAdapter.h"
...
CMpaDecModernDecodeAdapter::CMpaDecModernDecodeAdapter()
	: m_module(NULL)
	, m_session(NULL)
	, m_codec(0)
	, m_create(NULL)
...
bool CMpaDecModernDecodeAdapter::Open(uint32_t codec, const PlayasaFfmpegModernAudioOpenParams* params)
{
	Close();
	if (!codec || !params) {
		SetError("Invalid MpaDec modern open parameters");
		return false;
	}
	if (!LoadBridge()) {
		return false;
	...
	if (!m_create(codec, &m_session) || !m_session) {
		SetError("Failed to create MpaDec FFmpeg modern bridge session");
		return false;
	}
...
int CMpaDecModernDecodeAdapter::DecodeAudio(const unsigned char* data, size_t dataSize, int64_t pts, PlayasaFfmpegModernAudioFrameInfo* frameInfo)
{
	if (!m_session || !m_decodeAudio) {
		SetError("MpaDec FFmpeg modern bridge session is not open");
...
int CMpaDecModernDecodeAdapter::ReceiveAudio(PlayasaFfmpegModernAudioFrameInfo* frameInfo)
{
...
void CMpaDecModernDecodeAdapter::Flush()
{
...
void CMpaDecModernDecodeAdapter::Close()
{
...
bool CMpaDecModernDecodeAdapter::IsOpen() const
{
	return m_session != NULL;
}

uint32_t CMpaDecModernDecodeAdapter::OpenCodec() const
{
	return m_codec;
}

const char* CMpaDecModernDecodeAdapter::LastError() const
{
	return m_lastErrorText;
}

bool CMpaDecModernDecodeAdapter::LoadBridge()
{
	if (m_module) {
		return true;
	}
...
FARPROC CMpaDecModernDecodeAdapter::LoadRequiredProc(const char* name)
{
	FARPROC proc = GetProcAddress(m_module, name);
	if (!proc) {
		char message[256] = { 0 };
		_snprintf_s(message, sizeof(message), _TRUNCATE, "Missing MpaDec FFmpeg modern bridge export: %s", name);
		SetError(message);
	}
	return proc;
}

void CMpaDecModernDecodeAdapter::SetError(const char* message)
{
	if (!message) {
		message = "Unknown MpaDec FFmpeg modern bridge error";
	}
