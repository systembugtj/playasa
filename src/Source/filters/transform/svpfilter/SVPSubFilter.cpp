...
CSVPSubFilter::CSVPSubFilter(LPUNKNOWN lpunk, HRESULT* phr)
	: CBaseVideoFilter(NAME("SPlayer ��Ļ�˾�"), lpunk, phr, __uuidof(this), 1),
	m_fDoPreBuffering(1)
	, m_fFlip(0)
	, m_bDontUseThis(0)
...
HRESULT  CSVPSubFilter::GetDIB(BYTE* lpDib, DWORD* size, BOOL with_sub)
{
	...
	*size = required;
	...
	if (with_sub)
	{
		...
		AlphaBltSub(m_spd, rt_sub1, rt_sub2, fOutputFlipped);
	}
	// TODO: avoid embeded subtitle in snapshot by default
...
bool CSVPSubFilter::AlphaBltSub(SubPicDesc& spd, REFERENCE_TIME rt_sub1, REFERENCE_TIME rt_sub2, bool fFlipSub) {
	CAutoLock cAutoLock(&m_csQueueLock);
...
HRESULT CSVPSubFilter::Transform(IMediaSample* pIn)
{
	...
	AlphaBltSub(spd, rt_sub1, rt_sub2, fFlipSub);
	/*
	if( 0 )//interlaced
	{
...
void CSVPSubFilter::InitSubPicQueue()
{
	...
	if ((szName.Find(_T("SPlayer")) < 0) && (szName.Find(_T("����")) < 0)){
