...
#if 1
            if((m_timestamp != 0xffffffff && in_timestamp == m_timestamp+1) || m_follow_frame_time_stamp == in_timestamp ){
...
//
// CRealVideoDecoder (RFC-0032: modern FFmpeg bridge only)
//

namespace {

DWORD RealVideoFourccFromSubtype(const GUID& subtype)
{
	if (subtype == MEDIASUBTYPE_RV40 || subtype == MEDIASUBTYPE_RV41) {
		return MAKEFOURCC('R', 'V', '4', '0');
	}
	if (subtype == MEDIASUBTYPE_RV30) {
		return MAKEFOURCC('R', 'V', '3', '0');
	}
	if (subtype == MEDIASUBTYPE_RV20) {
		return MAKEFOURCC('R', 'V', '2', '0');
	}
	if (subtype == MEDIASUBTYPE_RV10) {
		return MAKEFOURCC('R', 'V', '1', '0');
	}
	return 0;
}

REFERENCE_TIME RealVideoAvgTimePerFrameFromMediaType(const CMediaType& mt)
{
	REFERENCE_TIME avgTimePerFrame = 0;
	if (mt.formattype == FORMAT_VideoInfo && mt.Format()) {
		avgTimePerFrame = reinterpret_cast<const VIDEOINFOHEADER*>(mt.Format())->AvgTimePerFrame;
	} else if (mt.formattype == FORMAT_VideoInfo2 && mt.Format()) {
		avgTimePerFrame = reinterpret_cast<const VIDEOINFOHEADER2*>(mt.Format())->AvgTimePerFrame;
	}
	return avgTimePerFrame > 0 ? avgTimePerFrame : 1;
}
...
, m_hModernBridge(NULL)
, m_modernSession(NULL)
, m_modernCodecFromFourcc(NULL)
, m_modernCreate(NULL)
, m_modernOpen(NULL)
, m_modernDecodeWithTiming(NULL)
, m_modernReceivePending(NULL)
, m_modernFlush(NULL)
, m_modernLastError(NULL)
, m_modernDestroy(NULL)
, m_tStart(0)
...
CRealVideoDecoder::~CRealVideoDecoder()
{
	FreeRV();
	if (m_hModernBridge) {
		FreeLibrary(m_hModernBridge);
		m_hModernBridge = NULL;
	}
}

FARPROC CRealVideoDecoder::LoadModernProc(const char* name)
{
	FARPROC proc = m_hModernBridge ? GetProcAddress(m_hModernBridge, name) : NULL;
	if (!proc) {
		char message[256] = { 0 };
		_snprintf_s(message, sizeof(message), _TRUNCATE, "Missing modern RealVideo bridge export: %s", name);
		SetModernError(message);
	}
	return proc;
}

void CRealVideoDecoder::SetModernError(const char* message)
{
	if (!message) {
		message = "Unknown modern RealVideo bridge error";
	}
	_snprintf_s(m_modernLastErrorText, sizeof(m_modernLastErrorText), _TRUNCATE, "%s", message);
}

const char* CRealVideoDecoder::GetModernError() const
{
	if (m_modernLastError && m_modernSession) {
		const char* errorText = m_modernLastError(m_modernSession);
		if (errorText && errorText[0]) {
			return errorText;
		}
	}
	return m_modernLastErrorText;
}

bool CRealVideoDecoder::LoadModernBridge()
{
	if (m_hModernBridge) {
		return true;
	}

	m_hModernBridge = LoadLibraryA("playasa_ffmpeg_modern_bridge.dll");
	if (!m_hModernBridge) {
		SetModernError("Failed to load playasa_ffmpeg_modern_bridge.dll");
		return false;
	}

	m_modernCodecFromFourcc = reinterpret_cast<PModernCodecFromFourcc>(LoadModernProc("playasa_ffmpeg_modern_codec_from_fourcc"));
	m_modernCreate = reinterpret_cast<PModernCreate>(LoadModernProc("playasa_ffmpeg_modern_create"));
	m_modernOpen = reinterpret_cast<PModernOpen>(LoadModernProc("playasa_ffmpeg_modern_open"));
	m_modernDecodeWithTiming = reinterpret_cast<PModernDecodeWithTiming>(LoadModernProc("playasa_ffmpeg_modern_decode_with_timing"));
	m_modernReceivePending = reinterpret_cast<PModernReceivePending>(LoadModernProc("playasa_ffmpeg_modern_receive_pending"));
	m_modernFlush = reinterpret_cast<PModernFlush>(LoadModernProc("playasa_ffmpeg_modern_flush"));
	m_modernLastError = reinterpret_cast<PModernLastError>(LoadModernProc("playasa_ffmpeg_modern_last_error"));
	m_modernDestroy = reinterpret_cast<PModernDestroy>(LoadModernProc("playasa_ffmpeg_modern_destroy"));

	if (!m_modernCodecFromFourcc || !m_modernCreate || !m_modernOpen || !m_modernDecodeWithTiming || !m_modernReceivePending || !m_modernFlush || !m_modernLastError || !m_modernDestroy) {
		FreeLibrary(m_hModernBridge);
		m_hModernBridge = NULL;
		return false;
	}
...
HRESULT CRealVideoDecoder::OpenModernRV(const CMediaType* pmt)
{
	const DWORD fourcc = RealVideoFourccFromSubtype(pmt->subtype);
	if (!fourcc) {
		return VFW_E_TYPE_NOT_ACCEPTED;
	}

	if (!LoadModernBridge()) {
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge load failed: %S", GetModernError());
		return E_FAIL;
	}

	uint32_t codec = 0;
	if (!m_modernCodecFromFourcc(fourcc, &codec)) {
		SetModernError("RealVideo codec is not supported by modern FFmpeg bridge");
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge codec unsupported fourcc=0x%08x", fourcc);
		return VFW_E_TYPE_NOT_ACCEPTED;
	}

	if (!m_modernCreate(codec, &m_modernSession) || !m_modernSession) {
		SetModernError("Failed to create modern RealVideo bridge session");
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge create failed: %S", GetModernError());
		return E_FAIL;
	}

	uint8_t* extradata = NULL;
	int extradata_size = 0;
	const bool hasExtradata = PlayasaBuildRealVideoExtradataFromVideoInfo(pmt->Format(), pmt->FormatLength(), &extradata, &extradata_size);
	if (!m_modernOpen(m_modernSession, hasExtradata ? extradata : NULL, hasExtradata ? static_cast<size_t>(extradata_size) : 0)) {
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge open failed: %S extradata=%d", GetModernError(), extradata_size);
		if (extradata) {
			free(extradata);
		}
		FreeRV();
	...
	m_modernLastErrorText[0] = '\0';
	SVP_LogMsg5(L"RealVideo modern FFmpeg bridge open OK fourcc=0x%08x extradata=%d", fourcc, extradata_size);
	return S_OK;
}
...
HRESULT CRealVideoDecoder::InitRV(const CMediaType* pmt)
{
	...
	return OpenModernRV(pmt);
}

void CRealVideoDecoder::FreeRV()
{
	if (m_modernSession && m_modernDestroy) {
		m_modernDestroy(m_modernSession);
	}
	m_modernSession = NULL;
	m_modernLastErrorText[0] = '\0';
}

HRESULT CRealVideoDecoder::DeliverModernFrame(
	IMediaSample* pIn,
	PlayasaFfmpegModernFrameInfo& frameInfo,
	REFERENCE_TIME inputDuration,
	REFERENCE_TIME& rtStart,
	DWORD realvideo_in_timestamp)
{
	if (!frameInfo.data[0]) {
		return S_OK;
	}
	...
	if (frameInfo.width <= 0 || frameInfo.height <= 0 || frameInfo.width > 4096 || frameInfo.height > 4096) {
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge invalid frame dimensions %d %d", frameInfo.width, frameInfo.height);
		return S_OK;
	}

	GUID subtype = MEDIASUBTYPE_I420;
	switch (frameInfo.pixel_format) {
	case PLAYASA_FFMPEG_MODERN_PIXFMT_PAL8:
		subtype = MEDIASUBTYPE_RGB8;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_BGR24:
	case PLAYASA_FFMPEG_MODERN_PIXFMT_RGB24:
		subtype = MEDIASUBTYPE_RGB24;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_RGB32:
		subtype = MEDIASUBTYPE_RGB32;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_YUVJ422P:
		subtype = MEDIASUBTYPE_YUVJ422P;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_YUVJ444P:
		subtype = MEDIASUBTYPE_YUVJ444P;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_YUV422P:
		subtype = MEDIASUBTYPE_YUV422P;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_YUV444P:
		subtype = MEDIASUBTYPE_YUV444P;
		break;
	case PLAYASA_FFMPEG_MODERN_PIXFMT_YUV420P:
	case PLAYASA_FFMPEG_MODERN_PIXFMT_YUVJ420P:
		subtype = MEDIASUBTYPE_I420;
		break;
	default:
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge unsupported pixel format %d", frameInfo.pixel_format);
		return VFW_E_INVALIDMEDIATYPE;
	}

	CComPtr<IMediaSample> pOut;
	BYTE* pDataOut = NULL;
	HRESULT hr = GetDeliveryBuffer(frameInfo.width, frameInfo.height, &pOut);
	...
	} else if (frameInfo.pts != PLAYASA_FFMPEG_MODERN_NO_PTS) {
		rtStart = frameInfo.pts;
	}

	REFERENCE_TIME frameDuration = frameInfo.duration > 0 ? frameInfo.duration : inputDuration;
	if (frameDuration <= 0) {
		frameDuration = m_rtAvrTimePerFrame;
	}
	if (frameDuration <= 0) {
		frameDuration = 1;
	}
	...
	BYTE* frameData[4] = {
		const_cast<BYTE*>(frameInfo.data[0]),
		const_cast<BYTE*>(frameInfo.data[1]),
		const_cast<BYTE*>(frameInfo.data[2]),
		const_cast<BYTE*>(frameInfo.data[3])
	};
	CopyBuffer(pDataOut, frameData, frameInfo.width, frameInfo.height, frameInfo.linesize[0], subtype, false);
	if (CComQIPtr<IMediaSample2> sample2 = pOut) {
		AM_SAMPLE2_PROPERTIES props;
		if (SUCCEEDED(sample2->GetProperties(sizeof(props), reinterpret_cast<BYTE*>(&props)))) {
			props.dwTypeSpecificFlags &= ~0x7f;
			props.dwTypeSpecificFlags |= AM_VIDEO_FLAG_WEAVE;
			sample2->SetProperties(sizeof(props), reinterpret_cast<BYTE*>(&props));
		}
	}
...
HRESULT CRealVideoDecoder::Transform(IMediaSample* pIn)
{
	...
	if (!m_modernSession || !m_modernDecodeWithTiming || !m_modernReceivePending) {
		return VFW_E_INVALIDMEDIATYPE;
	}
	...
	if (FAILED(pIn->GetTime(&rtStart, &rtStop))) {
		rtStart = PLAYASA_FFMPEG_MODERN_NO_PTS;
		rtStop = PLAYASA_FFMPEG_MODERN_NO_PTS;
	}
	const REFERENCE_TIME inputDuration = (rtStart != PLAYASA_FFMPEG_MODERN_NO_PTS && rtStop > rtStart) ? (rtStop - rtStart) : PLAYASA_FFMPEG_MODERN_NO_PTS;
	if (rtStart != PLAYASA_FFMPEG_MODERN_NO_PTS) {
		rtStart += m_tStart;
	}
	...
	PlayasaFfmpegModernFrameInfo frameInfo = { 0 };
	const int modernStatus = m_modernDecodeWithTiming(m_modernSession, pDataIn, nSize, rtStart, inputDuration, &frameInfo);
	if (modernStatus == PLAYASA_FFMPEG_MODERN_STATUS_FAILURE) {
		SVP_LogMsg5(L"RealVideo modern FFmpeg bridge decode failed: %S", GetModernError());
		return VFW_E_INVALIDMEDIATYPE;
	}
	if (modernStatus == PLAYASA_FFMPEG_MODERN_STATUS_FRAME_READY) {
		if (FAILED(hr = DeliverModernFrame(pIn, frameInfo, inputDuration, rtStart, rv_in_timestamp))) {
			return hr;
		}
	}

	for (;;) {
		PlayasaFfmpegModernFrameInfo pendingFrameInfo = { 0 };
		const int pendingStatus = m_modernReceivePending(m_modernSession, &pendingFrameInfo);
		if (pendingStatus == PLAYASA_FFMPEG_MODERN_STATUS_FAILURE) {
			SVP_LogMsg5(L"RealVideo modern FFmpeg bridge receive failed: %S", GetModernError());
			return VFW_E_INVALIDMEDIATYPE;
		}
		if (pendingStatus != PLAYASA_FFMPEG_MODERN_STATUS_FRAME_READY) {
			break;
		}
		if (FAILED(hr = DeliverModernFrame(pIn, pendingFrameInfo, inputDuration, rtStart, rv_in_timestamp))) {
			return hr;
		}
	}
...
HRESULT CRealVideoDecoder::CheckInputType(const CMediaType* mtIn)
{
	if (mtIn->majortype != MEDIATYPE_Video
		|| mtIn->subtype != MEDIASUBTYPE_RV10
	...
	if (mtIn->formattype == FORMAT_VideoInfo2) {
		const VIDEOINFOHEADER2* vih2 = reinterpret_cast<const VIDEOINFOHEADER2*>(mtIn->Format());
...
HRESULT CRealVideoDecoder::StartStreaming()
{
	...
	m_rtAvrTimePerFrame = RealVideoAvgTimePerFrameFromMediaType(mt);
	m_rtRVStart = 0;
	m_last_shown_timestamp = 0;
	m_rv_leap_frames = 0;
	m_rv_time_for_each_leap = 0;
...
{
	{
		...
		m_timestamp = ~0u;
		m_fDropFrames = false;
		if (m_modernSession && m_modernFlush) {
			m_modernFlush(m_modernSession);
		}
	}
