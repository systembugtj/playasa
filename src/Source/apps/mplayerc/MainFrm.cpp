...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND_RANGE(ID_VIEW_SEEKER, ID_VIEW_CONTROLS, OnViewControlBar)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_SEEKER, ID_VIEW_CONTROLS, OnUpdateViewControlBar)
	...
	ON_COMMAND_RANGE(ID_VIDEO_SUBITEM_START, ID_VIDEO_SUBITEM_END, OnPlayVideo)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIDEO_SUBITEM_START, ID_VIDEO_SUBITEM_END, OnUpdatePlayVideo)
...
CPlayerCaptureBar* CMainFrame::GetCaptureBar()
{
	return &m_wndCaptureBar;
}
CPlayerPlaylistBar* CMainFrame::GetPlaylistBar()
{
	return &m_wndPlaylistBar;
}
CPlayerSeekBar* CMainFrame::GetSeekBar()
{
	return &m_wndSeekBar;
}
CPlayerToolBar* CMainFrame::GetToolBar()
{
	return &m_wndToolBar;
}
CChildView* CMainFrame::GetVideoView()
{
	return &m_wndView;
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// load shaders
	CString	strList = AfxGetCurrentSettings().strShaderList;
	CString	strRes;
	int	curPos = 0;
	...
	*/
	...
	if (AfxGetCurrentSettings().fLoopForever)
...
void CMainFrame::OnMove(int x, int y)
{
	...
	if (!m_fFullScreen && wp.flags != WPF_RESTORETOMAXIMIZED && wp.showCmd != SW_SHOWMINIMIZED && m_WndSizeInited >= 2)
		GetWindowRect(AfxGetCurrentSettings().rcLastWindowPos);
...
{
	m_fTrayIcon = false;
	ShowTrayIcon(AfxGetCurrentSettings().fTrayIcon);
...
{
	SetAlwaysOnTop(AfxGetCurrentSettings().iOnTop);
...
void CMainFrame::OnMoving(UINT fwSide, LPRECT pRect)
{
	...
	if (AfxGetCurrentSettings().fSnapToDesktopEdges)
	{
...
void CMainFrame::OnSizing(UINT fwSide, LPRECT pRect)
{
	...
	// TODO
	{
		...
		if (!AfxGetCurrentSettings().fHideCaptionMenu) fsize.cy += 3;
...
void CMainFrame::OnDisplayChange() // untested, not sure if it's working...
{
	...
	//	AfxMessageBox(_T("SD"));
	if (m_iMediaLoadState == MLS_LOADED && m_pCAPR && !AfxGetCurrentSettings().fbSmoothMutilMonitor) {
...
void CMainFrame::OnPlaySubDelay(UINT nID)
{
	...
	case 0:
		newDelay = oldDelay - AfxGetCurrentSettings().nSubDelayInterval;
		break;
	case 2:
		newDelay = oldDelay + AfxGetCurrentSettings().nSubDelayInterval;
		break;
...
void CMainFrame::OnTimer(UINT nIDEvent)
{
	...
	case TIMER_IDLE_TASK:
	{
		...
		if (!s.bDontDeleteOldSubFileAutomaticly && (!IsSomethingLoaded() || GetMediaState() != State_Running))
		{
			...
			svpTool.CleanUpOldFiles(AfxGetCurrentSettings().GetSVPSubStorePath(true), 30, 5);
			SetTimer(TIMER_IDLE_TASK, 30000, NULL);
		}
	}
		break;
	...
	case TIMER_FULLSCREENCONTROLBARHIDER:
	{
		...
		if ((pWnd && (m_wndView == *pWnd || m_wndView.IsChild(pWnd) || fCursorOutside)) && (AfxGetCurrentSettings().nShowBarsWhenFullScreenTimeOut >= 0))
			ShowControls(CS_NONE, false);
	}
		break;
	...
	case TIMER_FULLSCREENMOUSEHIDER:
		...
		if (!IsMenuUp())
		{
			...
			if (AfxGetCurrentSettings().bUserAeroUI())
...
LRESULT CMainFrame::OnGraphNotify(WPARAM wParam, LPARAM lParam)
{
	...
	while (pME && SUCCEEDED(pME->GetEvent(&evCode, (LONG_PTR*)&evParam1, (LONG_PTR*)&evParam2, 0)))
	{
			...
			else if (m_wndPlaylistBar.GetCount() > 1)
			{
				if (m_nLoopSetting == ID_PLAYBACK_LOOP_CURRENT){
					...
					else
					{
						...
						if (s.fRewind)
						{
							AfxGetCurrentSettings().nCLSwitches |= CLSW_OPEN; // HACK
							PostMessage(WM_COMMAND, ID_NAVIGATE_SKIPFORWARD);
						}
						else
						{
		...
		else if (EC_VIDEO_SIZE_CHANGED == evCode)
		{
			...
			if (AfxGetCurrentSettings().fRememberZoomLevel
			...
			if (m_iMediaLoadState == MLS_LOADED
				&& !m_fAudioOnly && (AfxGetCurrentSettings().nCLSwitches&CLSW_FULLSCREEN))
			{
				PostMessage(WM_COMMAND, ID_VIEW_FULLSCREEN);
				AfxGetCurrentSettings().nCLSwitches &= ~CLSW_FULLSCREEN;
			}
		}
...
{
	if (!IsSomethingLoaded()){
		if (!AfxGetCurrentSettings().htpcmode)
			SendMessage(WM_COMMAND, ID_VIEW_FULLSCREEN);
		else
...
void CMainFrame::OnInitMenuPopup(CMenu * pPopupMenu, UINT nIndex, BOOL bSysMenu)
{
	...
	if (transl.IsEmpty())
	{
		...
		transl[_T("Video")] = IDS_VIDEO_POPUP;
	...
	for (UINT i = 0, j = pPopupMenu->GetMenuItemCount(); i < j; i++)
	{
		...
		else if (str == ResStr(IDS_VIDEO_POPUP))  // a file may contain various video streams
		{
			// let user can select various video streams
			CMenu *pVideoPopup = pPopupMenu->GetSubMenu(i);
			if (pVideoPopup)
			{
				SetupVideoMenu(pVideoPopup);
			}
			else
			{
				Logging(L"video popup is not exist");
			}
		}
...
// find video streams and create this popup menu
void CMainFrame::SetupVideoMenu(CMenu *pVideoMenu)
{
	// delete the old video stream menu
	if (pVideoMenu->GetMenuItemID(0) == ID_VIDEO_SUBITEM_START)
	{
		// remove video stream menu
		while (pVideoMenu->GetMenuItemID(0) != ID_SEPARATOR)
		{
			pVideoMenu->RemoveMenu(0, MF_BYPOSITION);
		}

		// remove the following separator
		if (pVideoMenu->GetMenuItemID(0) == ID_SEPARATOR)
		{
			pVideoMenu->RemoveMenu(0, MF_BYPOSITION);
		}
	}
	...
	if (pSS)
	{
		...
		for (DWORD i = 0; i < dwStreamCount; ++i)
		{
			AM_MEDIA_TYPE *pmt = 0;
			WCHAR *psName = 0;
			if (SUCCEEDED(pSS->Info(i, &pmt, 0, 0, 0, &psName, 0, 0)))
			{
				// video stream
	...
	// append info to the menu
	size_t nIndex = 0;
	for (nIndex = 0; nIndex < vtVideoStreamName.size(); ++nIndex)
	{
		// Note: must add MF_BYPOSITION, the first index is 0
		pVideoMenu->InsertMenu(nIndex, MF_ENABLED | MF_STRING | MF_BYPOSITION, id++, vtVideoStreamName[nIndex].c_str());
	}

	// append a separator
	if (vtVideoStreamName.size() > 0)
	{
		pVideoMenu->InsertMenu(nIndex, MF_SEPARATOR | MF_BYPOSITION);
	}
}
...
void CMainFrame::OnFilePostOpenmedia()
{
	...
	if (m_wndToolBar.IsVisible())
		ShowControls(AfxGetCurrentSettings().nCS | CS_SEEKBAR, false);
...

  {
	  ...
	  if (IsWindowVisible() && AfxGetCurrentSettings().fRememberZoomLevel
		  && !(m_fFullScreen || wp.showCmd == SW_SHOWMAXIMIZED || wp.showCmd == SW_SHOWMINIMIZED))
	  {
...
void CMainFrame::OnFilePostClosemedia()
{
	...
	//	AfxGetCurrentSettings().fEnableSubtitles2 = FALSE;
	m_iSubtitleSel2 = -1;
...
void CMainFrame::OnFileOpenQuick()
{
	...
	AfxGetCurrentSettings().Formats.GetFilter(filter, mask);
...
static CString MakeSnapshotFileName(LPCTSTR prefix)
{
	...
	CString fn;
	fn.Format(_T("%s%s%s"), prefix, t.Format(_T("%Y%m%d%H%M%S")), AfxGetCurrentSettings().SnapShotExt);
	return fn;
}
...
void CMainFrame::OnFileSaveImageAuto()
{
	...
	CString fn;
	fn.Format(_T("%s\\%s"), AfxGetCurrentSettings().SnapShotPath, MakeSnapshotFileName(_T("snapshot")));
...
{
	bool fHideCaptionMenu = AfxGetCurrentSettings().fHideCaptionMenu;

	AfxGetCurrentSettings().fHideCaptionMenu = !fHideCaptionMenu;
...
{
	pCmdUI->SetCheck(!AfxGetCurrentSettings().fHideCaptionMenu);
}

void CMainFrame::OnViewControlBar(UINT nID)
{
	nID -= ID_VIEW_SEEKER;
	ShowControls(AfxGetCurrentSettings().nCS ^ (1 << nID));
}

void CMainFrame::OnUpdateViewControlBar(CCmdUI* pCmdUI)
{
	UINT nID = pCmdUI->m_nID - ID_VIEW_SEEKER;
	pCmdUI->SetCheck(!!(AfxGetCurrentSettings().nCS & (1 << nID)));
}
...
{
	if (AfxGetCurrentSettings().fHideCaptionMenu)
		SendMessage(WM_COMMAND, ID_VIEW_CAPTIONMENU);
...
{
	if (IsSomethingLoaded())
		AfxGetCurrentSettings().nCS |= CS_SEEKBAR; // show seekbar when playback
	else
		AfxGetCurrentSettings().nCS &= ~CS_SEEKBAR; // hide seekbar when not playback
...
void CMainFrame::OnViewDefaultVideoFrame(UINT nID)
{
	...
	if (actionID == (AfxGetCurrentSettings().iDefaultVideoSize + ID_VIEW_VF_HALF)){
	...
	AfxGetCurrentSettings().iDefaultVideoSize = actionID - ID_VIEW_VF_HALF;
	m_ZoomX = m_ZoomY = 1;
	m_PosX = m_PosY = 0.5;
	MoveVideoWindow();
}
...
void CMainFrame::OnUpdateViewDefaultVideoFrame(CCmdUI* pCmdUI)
{
	...
	BOOL bCurrent = (AfxGetCurrentSettings().iDefaultVideoSize == (pCmdUI->m_nID - ID_VIEW_VF_HALF));
...
{
	AfxGetCurrentSettings().fKeepAspectRatio = !AfxGetCurrentSettings().fKeepAspectRatio;
	MoveVideoWindow();
}
...
{
	pCmdUI->Enable(m_iMediaLoadState == MLS_LOADED && !m_fAudioOnly);
	pCmdUI->SetCheck(AfxGetCurrentSettings().fKeepAspectRatio);
}

void CMainFrame::OnViewCompMonDeskARDiff()
{
	AfxGetCurrentSettings().fCompMonDeskARDiff = !AfxGetCurrentSettings().fCompMonDeskARDiff;
	MoveVideoWindow();
}
...
{
	pCmdUI->Enable(m_iMediaLoadState == MLS_LOADED && !m_fAudioOnly);
	pCmdUI->SetCheck(AfxGetCurrentSettings().fCompMonDeskARDiff);
}
...
{
	CSize& ar = AfxGetCurrentSettings().AspectRatio;
...
{
	pCmdUI->SetRadio(AfxGetCurrentSettings().AspectRatio == s_ar[pCmdUI->m_nID - ID_ASPECTRATIO_START]);
	pCmdUI->Enable(m_iMediaLoadState == MLS_LOADED && !m_fAudioOnly);
}
...
{
	nID -= ID_ONTOP_NEVER;
	if (AfxGetCurrentSettings().iOnTop == nID)
		nID = !nID;
	SetAlwaysOnTop(nID);
}
...
{
	pCmdUI->SetRadio(AfxGetCurrentSettings().iOnTop == (pCmdUI->m_nID - ID_ONTOP_NEVER));
}
...
void CMainFrame::OnPlayStopDummy(){
	...
	//if(AfxGetCurrentSettings().iSVPRenderType == 0){
...
{
	AfxGetCurrentSettings().i3DStereoKeepAspectRatio = !AfxGetCurrentSettings().i3DStereoKeepAspectRatio;
	MoveVideoWindow();
	Save3DStereoPerference();
}
void CMainFrame::OnUpdate3DStereoKeepAR(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(AfxGetCurrentSettings().i3DStereoKeepAspectRatio);
}
...
void CMainFrame::OnUpdate3DStereoControl(CCmdUI *pCmdUI)
{
	if (pCmdUI->m_nID >= ID_3DSTEREO_MENU_START && pCmdUI->m_nID <= ID_3DSTEREO_MENU_END)
		pCmdUI->SetCheck((pCmdUI->m_nID - ID_3DSTEREO_MENU_START + 1) == AfxGetCurrentSettings().i3DStereo);
}
void CMainFrame::OnPlayShaders(UINT nID)
{
	...
	else if (nID >= ID_SHADERS_START + 3)
	{
		...
		AfxGetCurrentSettings().m_shadercombine = Implode(m_shaderlabels, '|');
	}
...
void CMainFrame::OnPlayVideo(UINT nID)
{
	...
	{
		LONGLONG dwCurPos = 0;
		pMS->GetPositions(&dwCurPos, 0);

		// First, close the media
		SendMessage(WM_COMMAND, ID_FILE_CLOSEPLAYLIST);

		// Second, change the video track
		pSS->Enable(i, 0);

		// Third, replay the media
		SendMessage(WM_COMMAND, ID_PLAY_PLAY);
	}
}

void CMainFrame::OnUpdatePlayVideo(CCmdUI* pCmdUI)
{
	// 此处nVideoStreamIndex是指第几条视频流，而不是视频流真正的track number
	...
	if (pSS)
	{
		...
		DWORD dwVideoStreamTrackIndex = -1;
		DWORD dwVideoStreamCount = -1;
		// look for the real video stream track
		for (size_t i = 0; i < dwStreamCount; ++i)
		{
			// get media type
			AM_MEDIA_TYPE *pmt = 0;
			pSS->Info(i, &pmt, 0, 0, 0, 0, 0, 0);

			if (pmt && (pmt->majortype == MEDIATYPE_Video))
			{
				++dwVideoStreamCount;

				if (dwVideoStreamCount == nVideoStreamIndex)
				{
					dwVideoStreamTrackIndex = i;
		...
		// to see if this video stream is enable
		DWORD flags;
		if (SUCCEEDED(pSS->Info(dwVideoStreamTrackIndex, 0, &flags, 0, 0, 0, 0, 0)))
		{
...
void CMainFrame::OnPlaySubtitles(UINT nID)
{
	...
	{   //字幕风格设置
		...
		{
			CComPtr<ISubStream> pSubStream;
			if (secondSub){ pSubStream = m_pSubStreams2.GetNext(pos); }
			else{ pSubStream = m_pSubStreams.GetNext(pos); }
			if (i < pSubStream->GetStreamCount())
			{
				CLSID clsid;
				if (FAILED(pSubStream->GetClassID(&clsid)))
	...
	if (secondSub){
		AfxGetCurrentSettings().fEnableSubtitles2 = !(m_iSubtitleSel2 & 0x80000000);
	}
	else{
		AfxGetCurrentSettings().fEnableSubtitles = !(m_iSubtitleSel & 0x80000000);
	}
}
...
void CMainFrame::OnFavoritesFile(UINT nID)
{
	...
	CAtlList<CString> sl;
	AfxGetCurrentSettings().GetFav(FAV_FILE, sl);
...
void CMainFrame::OnFavoritesDVD(UINT nID)
{
	...
	CAtlList<CString> sl;
	AfxGetCurrentSettings().GetFav(FAV_DVD, sl);
...
{
	bool fCompMonDeskARDiff = AfxGetCurrentSettings().fCompMonDeskARDiff;
	CSize ret = CGraphCore::GetVideoSize();
...
void CMainFrame::ToggleFullscreen(bool fToNearest, bool fSwitchScreenResWhenHasTo)
{
	...
	if (!m_fFullScreen)
	{
		...
		dispmode& dm = AfxGetCurrentSettings().dmFullscreenRes;
	...
	else
	{
		...
		dwAdd = (AfxGetCurrentSettings().fHideCaptionMenu ? 0 : WS_CAPTION) | WS_THICKFRAME;
		r = m_lastWindowRect;
		hMenu = NULL;//AfxGetCurrentSettings().fHideCaptionMenu ? NULL : m_hMenuDefault;
	}
	...
	else
	{
		...
		m_fHideCursor = false;
		ShowControls(AfxGetCurrentSettings().nCS);
	}
...
UINT CMainFrame::GetBottomSubOffset(){
	if (m_fFullScreen || AfxGetCurrentSettings().fHideCaptionMenu){
		CRect vr, mr;
...
{
//	m_wndToolBar.Volume = AfxGetCurrentSettings().nVolume;
//	SetBalance(AfxGetCurrentSettings().nBalance);
}
*/
...
void CMainFrame::SetupOpenCDSubMenu()
{
	...
	if (AfxGetCurrentSettings().fHideCDROMsSubMenu) return;
...
CString CMainFrame::GetAnEasyToUnderstoodAudioStreamName(CString szName)
{
	...
	switch (AfxGetCurrentSettings().iLanguage)
	{
	case 0:
	case 2:
...
void CMainFrame::SetupSubtitlesSubMenu(int subid)
{
	...
	{
		CComPtr<ISubStream> pSubStream;
		if (subid == 2){ pSubStream = m_pSubStreams2.GetNext(pos); }
		else { pSubStream = m_pSubStreams.GetNext(pos); }
		if (!pSubStream) continue;
...
void CMainFrame::SetupFavoritesSubMenu()
{
	...
	AfxGetCurrentSettings().GetFav(FAV_DEVICE, sl);
...
void CMainFrame::SetupShadersSubMenu()
{
	...
	if (POSITION pos = AfxGetCurrentSettings().m_shaders.GetHeadPosition())
	{
		...
		while (pos)
		{
			const AppSettings::Shader& s = AfxGetCurrentSettings().m_shaders.GetNext(pos);
...
void CMainFrame::OnUpdateChangeResizer(CCmdUI* pCmdUI){
	...
	case ID_RESIZER_NORMAL:
		pCmdUI->SetRadio(AfxGetCurrentSettings().iDX9Resizer == 1);
		break;
	case ID_RESIZER_SMARTBICUBIC:
		pCmdUI->SetRadio(AfxGetCurrentSettings().iDX9Resizer == 7);
		break;
	}
}
void CMainFrame::OnChangeResizer(UINT nID){
	switch (nID){
	case ID_RESIZER_NORMAL:
		AfxGetCurrentSettings().iDX9Resizer = 1;
		return;
	case ID_RESIZER_SMARTBICUBIC:
		AfxGetCurrentSettings().iDX9Resizer = 7;
		return;
	}
...
{
	if (setSetting){
		AfxGetCurrentSettings().iOnTop = i;
	}
...
bool CMainFrame::StopCapture()
{
	...
	::SetPriorityClass(::GetCurrentProcess(), AfxGetCurrentSettings().priority);
...
void CMainFrame::OnUpdateSmartDragEnable(CCmdUI *pCmdUI){
	pCmdUI->SetCheck(!!(AfxGetCurrentSettings().useSmartDrag));
}
...
{
	pCmdUI->SetCheck(!!(AfxGetCurrentSettings().autoDownloadSVPSub));
}


void CMainFrame::OnVisitShop()
{
	if (AfxGetCurrentSettings().szOEMStoreLink.IsEmpty())  //Visit BBS
		ShellExecute(m_hWnd, _T("open"), _T("http://shop.shooter.cn"), NULL, NULL, SW_SHOWDEFAULT);
	else
		ShellExecute(m_hWnd, _T("open"), AfxGetCurrentSettings().szOEMStoreLink, NULL, NULL, SW_SHOWDEFAULT);
}
...
void CMainFrame::OnCheckAndSetDefaultPlayer()
{
	...
	if (action_id & 1 << 3)
	{
		...
		{
			if (AfxGetCurrentSettings().fPopupStartUpExtCheck || (CMPlayerCApp::IsVista() && !IsUserAnAdmin()))
			{
				m_chkdefplayercontrolbar.ShowWindow(SW_SHOWNOACTIVATE);
				rePosOSD();
			}
...
void CMainFrame::_HandleTimer_StreamPosPoller()
{
	...
	if (m_pCAP && (m_iPlaybackMode != PM_FILE || m_bMustUseExternalTimer))
	{
		...
		AfxGetCurrentSettings().bExternalSubtitleTime = true;
		if (pDVDI)
		{
			DVD_PLAYBACK_LOCATION2 Location;
	...
	else
	{
		...
		AfxGetCurrentSettings().bExternalSubtitleTime = false;
	}
}
