...
BOOL CShaderEditorDlg::Create(CWnd* pParent)
{
	...
	POSITION pos = AfxGetCurrentSettings().m_shaders.GetHeadPosition();
	while(pos)
	{
...
void CShaderEditorDlg::OnCbnSelchangeCombo1()
{
	...
	if(i < 0)
	{
		...
		POSITION pos = AfxGetCurrentSettings().m_shaders.AddTail(s);

		i = m_labels.AddString(s.label);
		m_labels.SetCurSel(i);
		m_labels.SetItemDataPtr(i, (void*)&AfxGetCurrentSettings().m_shaders.GetAt(pos));
	}
