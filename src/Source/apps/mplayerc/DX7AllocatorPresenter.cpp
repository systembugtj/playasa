...
HRESULT CDX7AllocatorPresenter::CreateDevice()
{
	...
	if (AfxGetCurrentSettings().fbSmoothMutilMonitor)
	...
	{
		m_pAllocator = new CDX7SubPicAllocator(m_pD3DDev, size, AfxGetCurrentSettings().fSPCPow2Tex);
...
HRESULT CRM7AllocatorPresenter::AllocSurfaces()
{
	...
	if (AfxGetCurrentSettings().bRGBOnly){
		m_pVideoSurfaceYUY2 = NULL;
	}
	else{
