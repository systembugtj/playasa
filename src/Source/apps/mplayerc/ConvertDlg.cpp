...
//	if(AfxGetCurrentSettings().fEnableWebServer) m.AppendMenu(MF_STRING, 1000, ResStr(IDS_CONVERT_LAUNCHINBROWSER));
	m.AppendMenu(MF_SEPARATOR);
	...
	case 1000:
		{
			CString url;
			url.Format(_T("http://localhost:%d/convres.html?id=%x"), AfxGetCurrentSettings().nWebServerPort, (DWORD)&t->m_res);
