...
HRESULT CFGManager::EnumSourceFilters(LPCWSTR lpcwstrFileName, CFGFilterList& fl)
{
    ...
    if(hFile != INVALID_HANDLE_VALUE && CheckBytes(hFile, L"0,4,,7776706B"))
    {
      ...
      else if (ext.CompareNoCase(L".wvc") == 0)
      {
		...
		if(!ext.IsEmpty() && !s.fBUltraFastMode)  //����ģʽ
		{
			// file extension
			CRegKey key;
...
HRESULT CFGManager::AddSourceFilter(CFGFilter* pFGF, LPCWSTR lpcwstrFileName, LPCWSTR lpcwstrFilterName, IBaseFilter** ppBF)
{
	...
	AfxGetCurrentSettings().szFGMLog.AppendFormat(_T("\r\nFGM: AddSourceFilter trying '%s' %s\n"), CStringFromGUID(pFGF->GetCLSID()) , pFGF->GetName());
	...
	: CFGManager(pName, pUnk)
{
  ...
  m_source.AddTail(pFGF);
   ...
   if(0) {
	...
	//if(AfxGetCurrentSettings().fUseWMASFReader)
	{
...
*/
  ...
  m_transform.AddTail(pFGF);
	...
	m_transform.AddTail(pFGF);
 		...
 		//if(src & SRC_MPEG) {
 	 		pFGF = new CFGFilterInternal<CMpegSplitterFilter>(L"Mpeg ������", MERIT64_ABOVE_DSHOW + 1000);
 	 	//} else {
 	 	//	pFGF = new CFGFilterInternal<CMpegSplitterFilter>(L"Mpeg ������ (���÷���)", MERIT64_UNLIKELY);
 	 	//}
...
*/
	...
	pFGF->AddType(MEDIATYPE_MPEG2_PACK, MEDIASUBTYPE_MPEG2_VIDEO);
	pFGF->AddType(MEDIATYPE_MPEG2_PES, MEDIASUBTYPE_MPEG2_VIDEO);
	pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_MPEG);
	pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_MPEG2_VIDEO);
	//pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_MMES);
	m_transform.AddTail(pFGF);
  ...
  m_transform.AddTail(pFGF);
    ...
    pFGF->AddType(MEDIATYPE_DVD_ENCRYPTED_PACK, MEDIASUBTYPE_DOLBY_DDPLUS);
    pFGF->AddType(MEDIATYPE_MPEG2_PACK, MEDIASUBTYPE_DOLBY_DDPLUS);
    pFGF->AddType(MEDIATYPE_MPEG2_PES, MEDIASUBTYPE_DOLBY_DDPLUS);
  ...
  pFGF = new CFGFilterInternal<CMPCVideoDecFilter>(_T("SVP FRAPS Video Decoder"), MERIT64_ABOVE_DSHOW);
  pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_FRAPS1);
  m_transform.AddTail(pFGF);
...
/*
  ...
  pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_MJPA);
	...
	{
		pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_WVC1); //��֧�ָ���VC1 :(
		pFGF->AddType(MEDIATYPE_Video, MEDIASUBTYPE_wvc1);
	}
#endif
...
// 		szaExtFilterPaths.Add( svptoolbox.GetPlayerPath(_T("csfcodec\\mpc_wtlvcl.dll")) );
  ...
  m_transform.AddTail(new CFGFilterRegistry(GUIDFromCString(_T("{2DE1D17E-46B1-42A8-9AEC-E20E80D9B1A9}")), MERIT64_ABOVE_DSHOW+300) ); 
	...
	for(int l = 0; l < szaExtFilterPaths.GetCount(); l++){
		CString szFPath = szaExtFilterPaths.GetAt(l); //���ļ�ģʽ���������
		CString szLog ; 
...
/*
	...
	CInterfaceList<IUnknown, &IID_IUnknown> pUnks;
	HRESULT d3dhr;
	CFGFilterVideoRenderer* pFGRVMR9 = new CFGFilterVideoRenderer(m_hWnd, CLSID_VMR9AllocatorPresenter, L"DX9(VMR)��Ⱦ��", m_vrmerit);
	CFGFilterVideoRenderer* pFGREVR;
	if ( (CMPlayerCApp::IsVista()  ) && !s.bDisableEVR ) {
		pFGREVR = new CFGFilterVideoRenderer(m_hWnd, CLSID_EVRAllocatorPresenter, L"EVR��Ⱦ��", m_vrmerit+1);
	...
	if(s.bIsIVM){
		...
		m_transform.AddTail(new CFGFilterVideoRenderer(m_hWnd, CLSID_VideoMixingRenderer, L"IVM��Ⱦ��(DX7)", m_vrmerit+10));
		m_transform.AddTail(new CFGFilterVideoRenderer(m_hWnd, CLSID_VideoMixingRenderer9, L"IVM��Ⱦ��(DX9)", m_vrmerit + ( (s.iDSVideoRendererType == VIDRNDT_DS_VMR9RENDERLESS) ? 12 : 8 ) ));
	}
	...
	/*
				...
				m_transform.AddTail(new CFGFilterVideoRenderer(m_hWnd, CLSID_VMR7AllocatorPresenter, L"DX7(VMR)��Ⱦ��", m_vrmerit));*/
	}
