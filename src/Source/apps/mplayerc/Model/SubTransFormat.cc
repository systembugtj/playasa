...
int SubTransFormat::ExtractSubFiles(FILE* fp, std::vector<std::wstring> &tmpfiles)
{
	...
	size_t iFileDataLength = CHAR4TOINT(szSBuff); //ȷ���ʵ����
	if (!iFileDataLength)
		return 0;
	...
	for (size_t i = 0; i < szSubfiles.size(); i++)
	{
		...
		while (IfFileExist_STL(szTarget))
		{
			...
			if (szTargetMD5 == szSourceMD5)
			{
				...
				ialreadyExist++; //TODO: �idx+sub��ֻ�һ�ļ�ͬ�ô�~~ 
				is_samefile = true;
				break;
			}
...
#ifdef CLIENTKEY	
	sprintf_s(buffx, 4096, CLIENTKEY, SVP_REV_STR, szTerm2, szTerm3, uniqueIDHash);
#else
