...
bool PlaylistParser::GetPlaylistItem(std::wstring fn,
	std::vector<std::wstring>* subs,
	CPlaylistItem* newPli)
{
	std::vector<std::wstring> fns;
	fns.push_back(fn);
	return GetPlaylistItem(fns, subs, newPli);
}

bool PlaylistParser::GetPlaylistItem(std::vector<std::wstring>& fns,
	std::vector<std::wstring>* subs,
	CPlaylistItem* newPli)
{
		...
		iter != fns.end(); iter++)
	{
		...
		if (!(*iter).empty())
			newPli->m_fns.AddTail((*iter).c_str());
	}
	...
	if (subs)
	{
			...
			iter != fns.end(); iter++)
		{
			...
			(*iter).erase(0, (*iter).find_first_not_of(L' '));
			if (!(*iter).empty())
				newPli->m_subs.AddTail((*iter).c_str());
		}
	}

	if (newPli->m_fns.IsEmpty())
		return false;
	...
	if (pref->GetIntVar(INTVAR_AUTOLOADAUDIO) && (fn.find(L"://") == fn.npos))
	{
		...
		if (dotposfn != fn.npos)
		{
			CMediaFormats& mf = AfxGetCurrentSettings().Formats;
	...
	std::vector<std::wstring>* subs)
{
		...
		iter != szFnsInRar.end(); iter++)
	{
		...
		{
			if (GetPlaylistItem(L"rar://" + fn + L"?" + szThisFn, subs, &newPli))
	...
	std::vector<std::wstring>* subs)
{
	...
	// resolve rar file
	{
			...
			iter != fns.end(); iter++)
		{
			...
			if ((*iter).substr(0, 6) == L"rar://")
				continue;
			std::wstring ext = CPath((*iter).c_str()).GetExtension();
			std::transform(ext.begin(), ext.end(), ext.begin(), tolower);
			if (ext == L".rar")
			{
		...
		iter != fns.end() && pSL && pPF; iter++)
	{
		...
		TCHAR buff[MAX_PATH];
		if (ext != _T(".lnk")
			|| FAILED(pPF->Load(CStringW((*iter).c_str()), STGM_READ))
	...
	CPlaylistItem newPli;
	if (GetPlaylistItem(fns, subs, &newPli))
...
bool PlaylistParser::FindFileInList(CAtlList<CString>& sl, std::wstring fn)
{
	bool fFound = false;
