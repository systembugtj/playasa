...
void NetworkControlerImpl::SinetConfig(sinet::refptr<sinet::config> cfg, int retryid)
{
  ...
  if(m_oemtitle.empty())
    wsprintf(agentbuff, L"SPlayer Build %d %s", SVP_REV_STR, splayer_uuid.c_str());
  else
    wsprintf(agentbuff, L"SPlayer Build %d OEM%s %s", SVP_REV_STR,m_oemtitle.c_str(), splayer_uuid.c_str());
