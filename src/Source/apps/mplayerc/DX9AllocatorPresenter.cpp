...
{
	AppSettings& s = AfxGetCurrentSettings();
	AppSettings::CRendererSettingsEVR & New = AfxGetCurrentSettings().m_RenderSettings;
...
//SVP_LogMsg5(L"CDX9AllocatorPresenter::CreateDevice end");
	...
	CSize size;
	switch(AfxGetCurrentSettings().nSPCMaxRes)
	{
	...
	{
		m_pAllocator = new CDX9SubPicAllocator(m_pD3DDev, size, AfxGetCurrentSettings().fSPCPow2Tex);
        if(!m_pAllocator){
	...
	hr = S_OK;
	m_pSubPicQueue = AfxGetCurrentSettings().nSPCSize > 0 
		? (ISubPicQueue*)new CSubPicQueue(AfxGetCurrentSettings().nSPCSize, m_pAllocator, &hr)
	...
	HRESULT hr2 = S_OK;
	m_pSubPicQueue2 = AfxGetCurrentSettings().nSPCSize > 0 
		? (ISubPicQueue*)new CSubPicQueue(AfxGetCurrentSettings().nSPCSize, m_pAllocator, &hr2)
	...
	//if(s.fResetDevice)
	{
		...
		if(SUCCEEDED(m_pD3DDev->GetCreationParameters(&Parameters))){
			...
			HMONITOR hCurMonitor = m_pD3D->GetAdapterMonitor(CurrentMonitor);
				...
				{
					if(!AfxGetCurrentSettings().fbSmoothMutilMonitor/*s.fResetDevice*/)
					{
...
{
	if(!AfxGetCurrentSettings().fVMRGothSyncFix){
...
HRESULT CDXRAllocatorPresenter::SetDevice(IDirect3DDevice9* pD3DDev)
{
	...
	{
		m_pAllocator = DNew CDX9SubPicAllocator(pD3DDev, size, AfxGetCurrentSettings().fSPCPow2Tex);
		if(!m_pAllocator)
			return E_FAIL;
	}
	...
	m_pSubPicQueue = AfxGetCurrentSettings().nSPCSize > 0 
		? (ISubPicQueue*)DNew CSubPicQueue(AfxGetCurrentSettings().nSPCSize, m_pAllocator, &hr)
