...
bool CGraphCore::OpenMediaPrivate(CAutoPtr<OpenMediaData> pOMD)
{
...

	{
		...
		try
		{
			...
			if (!(AfxGetCurrentSettings().nCLSwitches&CLSW_OPEN))
				GetMainFrame()->PostMessage(WM_COMMAND, ID_PLAY_PLAY);

			AfxGetCurrentSettings().nCLSwitches &= ~CLSW_OPEN;
...
void CGraphCore::OpenCustomizeGraph()
{
	...
	{
		if (m_pCAP) {
			if (AfxGetCurrentSettings().fAutoloadSubtitles) {
	...
	BeginEnumFilters(pGB, pEF, pBF)
	{
		if (GetCLSID(pBF) == CLSID_OggSplitter)
		{
			...
			{
				LCID idAudio = AfxGetCurrentSettings().idAudioLang;
				if (!idAudio) idAudio = GetUserDefaultLCID();
				LCID idSub = AfxGetCurrentSettings().idSubtitlesLang;
...
void CGraphCore::OpenSetupAudio()
{
	...
	// FIXME
	int balance = AfxGetCurrentSettings().nBalance;
...
void CGraphCore::OpenMedia(CAutoPtr<OpenMediaData> pOMD)
{
	...
	if (m_pGraphThread && fUseThread
		&& AfxGetCurrentSettings().fEnableWorkerThreadForOpening)
	{
...
{
	bool fKeepAspectRatio = AfxGetCurrentSettings().fKeepAspectRatio;
...
{
	AfxGetCurrentSettings().nBalance = balance;
