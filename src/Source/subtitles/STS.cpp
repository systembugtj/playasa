	...
	while(file->ReadString(buff))
	{
		...
		else if(c == 14) // time info
		{
			...
			while(file->ReadString(tmp))
			{
				...
				//tmp.Trim(L"¡¡ \t\n\r"); // support chinese space character
				//tmp.Trim();
				if(tmp.IsEmpty()) fFoundEmpty = true;

				tmp2 = tmp;
				tmp2.Trim(L"¡¡ \t\n\r"); //dame 很难正确trim中文的空格符
				if(tmp2.IsEmpty()){ fFoundEmpty = true; tmp.Empty();}

				int num2;
				WCHAR c;
				if( fFoundEmpty && !tmp.IsEmpty())
				{
					int xret = swscanf(tmp, L"%d%c", &num2, &c);
					if(xret == 1){
					num = num2;
					break;
					}else{
...
if(version >= 6)marginRect.bottom = GetInt(buff);
				...
				Style.TrimLeft('*');
				if(!Style.CompareNoCase(_T("SVPDefault"))) Style = _T("SVPDefault");
...
void CSimpleTextSubtitle::Add(CStringW str, bool fUnicode, int start, int end, CString style, CString actor, CString effect, CRect marginRect, int layer, int readorder)
{
	...
	str.Remove('\r');
	str.Replace(L"\n", L"\\N");
	if(style.IsEmpty()) style = _T("SVPDefault");
	style.TrimLeft('*');
		...
		if(!m_styles.Lookup(stse.style, val))
		{
			if(!unknown.Lookup(stse.style, val))
			{
				if(fReport)
				{
					...
					fReport = false;//if(MessageBox(NULL, msg, _T("Warning"), MB_OKCANCEL|MB_ICONWARNING) != IDOK) fReport = false;
				}

				unknown[stse.style] = NULL;
			}

			stse.style = _T("SVPDefault");
		}
	}
}
...
void CSimpleTextSubtitle::AddStyle(CString name, STSStyle* style)
{
	...
	if(name.IsEmpty()) name = _T("SVPDefault");
...
bool CSimpleTextSubtitle::SetDefaultStyle(STSStyle& s,  bool overideCharSet)
{
	STSStyle* val;
	if(!m_styles.Lookup(_T("SVPDefault"), val)) return false;
	if (!overideCharSet){
		s.charSet = val->charSet;
	}
	*val = s;
	m_fUsingAutoGeneratedDefaultStyle = false;
	return true;
}
CRect rotateCRect(CRect r, int iAngle){
	LONG swapPos = 0;
	switch(iAngle){
		case 4: //顺时针90度
			swapPos = r.top;
      ...
      continue;
		if(style){
			if(iAlign >= 0){
				...
				posRect = rotateCRect(posRect, iAlign);
			}
			if(!posRect.IsRectNull())
				style->marginRect = posRect;
		}
...
//	Sort();
	...
	STSStyle* s;
	if(!m_fUsingAutoGeneratedDefaultStyle && m_styles.Lookup(_T("SVPDefault"), s) && et != EXTSSA && et != EXTASS)
	{
		CTextFile f;
		...
		str  = _T("Style: SVPDefault,%s,%d,&H%08x,&H%08x,&H%08x,&H%08x,%d,%d,%d,%d,%d,%d,%d,%.2f,%d,%d,%d,%d,%d,%d,%d,%d\n");
		str2.Format(str,
			s->fontName, (int)s->fontSize,
			(s->colors[0]&0xffffff) | (s->alpha[0]<<24),
			(s->colors[1]&0xffffff) | (s->alpha[1]<<24),
