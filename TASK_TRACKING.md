...
| 进行中 | **RFC-0013**：Rust 原生模块接入试点（`sphash`） | 根 `Cargo.toml` workspace + `crates/sphash` 已接入；Rust 单测 4/4 通过；`mplayerc` Release Unicode 与 `Updater` Release 的 `BuildPlayasaRustSphash` 目标通过并产出 `playasa_sphash.dll` + import `.lib`；完整 C++ 链接/运行 smoke test 待继续 |
|  |  |  |
