...
**当前约定（2026）**：主解决方案为 `src\splayer.sln`，工程路径相对 `src\`。下文仍保留「迁至仓库根」的历史讨论，便于对照。
...
1. **解决方案文件位置（团队取舍）**
   - 当前：`splayer.sln` 在 `src/` 下，与脚本、MSBuild 用法一致
   - 备选：迁至仓库根目录（需改写路径前缀），见下文「选项 A」
